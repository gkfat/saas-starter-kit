...
The system SHALL verify the LIFF-issued ID Token (JWT) on the server by validating its signature, `aud` (LINE channel id), and `exp` claims before trusting any LINE identity in the token. The system SHALL NOT call LINE's profile API as part of this verification.

#### Scenario: Valid LIFF ID Token

- **WHEN** the LIFF frontend submits a LIFF ID Token to the backend
- **THEN** system verifies the token's signature, `aud`, and `exp`, and extracts the LINE `provider_user_id` from its claims

#### Scenario: Invalid or expired LIFF ID Token

- **WHEN** the LIFF frontend submits a LIFF ID Token that fails signature, `aud`, or `exp` validation
