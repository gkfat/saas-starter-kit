# Spec: Profile Self-Service

## Purpose

提供個人資料頁的自助服務功能，讓使用者自行編輯 displayName、查看帳號綁定狀態（Email、手機、Google 登入），並可自行完成手機驗證與 Google 帳號綁定/解除綁定，皆遵循既有 thin API → service → repo 分層與 i18n 慣例。
