# Member Level Management Spec

## Purpose

Provides a metric-agnostic member level (tier) system: per-member evaluation periods anchored to signup date, immediate mid-period upgrades with period-end-only downgrades, an immutable metric ledger, Firestore-backed tier configuration with an admin UI, and a scheduler-triggered due-period evaluation batch. Fully decoupled from the `points` module and any specific metric source.
